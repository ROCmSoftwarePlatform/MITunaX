...
def finApplicability(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8") {
        ...
        sh "./tuna/go_fish.py miopen import_configs -t recurrent_${branch_id}_nchw --mark_recurrent -f utils/configs/conv_configs.txt --model Resnet50 --md_version 1 --framework Pytorch --fw_version 1"
...
def finFindEval(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8 ${docker_args}") {
        ...
        gpu_list.each{
            celery_log="${env.WORKSPACE}/tuna/${branch_id}_find_eval_celery_log_${counter}.log"
            sh "touch ${celery_log}"
            def proc_id = sh(script: "celery -A tuna.celery_app.celery_app worker -l debug --logfile=${celery_log} -n tuna_${branch_id}_gpu_id_${counter} -Q eval_q_${db_name}_sess_${sesh2} -c 1 2>\0461 1>/dev/null & echo \$!", returnStdout: true).trim()
            sh "cat ${celery_log}"
            pid_list.add(proc_id)
            counter++
        }
